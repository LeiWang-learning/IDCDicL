...
class Model:
    ...
    def load_network(self, load_path: str, network: Union[nn.DataParallel,
        ...
        if self.opt['train']['reload_broadcast']:
            ...
            for hypa in network.hypa_list:
                hypa.load_state_dict(state_dict_hypa_v2, strict=True)
        else:
    ...
    def define_loss(self):
        ...
        self.ssim_loss = pytorch_mssim.mssim
...
#        self.sigma = data['sigma'].to(self.device)
    ...
    def cal_multi_loss(self, preds: List[torch.Tensor],
        ...
        for i, pred in enumerate(preds):
            loss1 = self.lossfn(pred, gt)
            loss2 = 1 - self.ssim_loss(pred, gt)
...
#            loss = loss1
    ...
    def cal_metrics(self):
        self.metrics['psnr'] = util.calculate_psnr(self.out_dict['dx'],self.out_dict['y_gt'])
        self.metrics['ssim'] = util.calculate_ssim(self.out_dict['dx'],self.out_dict['y_gt'])
        self.metrics['psnr_LD'] = util.calculate_psnr(self.out_dict['y'], self.out_dict['y_gt'])
        self.metrics['ssim_LD'] = util.calculate_ssim(self.out_dict['y'], self.out_dict['y_gt'])

        return self.metrics['psnr'], self.metrics['ssim'], self.metrics['psnr_LD'], self.metrics['ssim_LD']
    ...
    def save_visuals(self, tag: str):
        ...
        img_path = os.path.join(img_dir,f"{img_name}_{tag}_{round(self.metrics['psnr'],4)}_{round(self.metrics['ssim'],4)}.png")
        img_path_LD = os.path.join(img_dir,f"{img_name}_{tag}_{round(self.metrics['psnr_LD'],4)}_{round(self.metrics['ssim_LD'],4)}.png")
        img_path_full = os.path.join(img_dir,f"{img_name}_full.png")

        util.imsave(dx_img, img_path)

        if self.opt['test']['visualize']:
            util.save_d(d_img.mean(0).numpy(), img_path.replace('.png', '_d.png'))
            util.imsave(y_img, img_path_LD.replace('.png', '_LD.png'))
            util.imsave(gt_img,img_path_full.replace('.png','_GT.png'))

    def train(self):
        self.optimizer.zero_grad()
        dxs, self.d = self.net(self.y)
