...
def augment_img(img: np.ndarray, mode: int = 0) -> np.ndarray:
    ...
    if mode == 0:
        ...
        return np.flipud(np.rot90(img)),mode
    elif mode == 2:
        return np.flipud(img),mode
    elif mode == 3:
        return np.rot90(img, k=3),mode
    elif mode == 4:
        return np.flipud(np.rot90(img, k=2)),mode
    elif mode == 5:
        return np.rot90(img),mode
    elif mode == 6:
        return np.rot90(img, k=2),mode
    elif mode == 7:
        return np.flipud(np.rot90(img, k=3)),mode
    else:
        raise ValueError
